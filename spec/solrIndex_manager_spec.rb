    ...
    module Kernel
      def `(cmd)
        ...
        if cmd.start_with? 'hadoop fs -du'
          return itr.map { |n| "#{n*10000000000} solrindex/test_index/part-r-0000#{n}" }.join "\n"
        elsif cmd.start_with? 'hadoop fs -cat'
          return "key '2011#{/\d+/.match(cmd).to_s.to_i}0': 62700332 documents"
        elsif cmd.start_with? 'ls /merge_to/test'
          return itr.map {|n| "20110#{n}0"}.join("\n")
